...
let diamanten = 0, shopUpgradeLevel = 0, shopUpgradePrijs = 1;
...
window.genereerMissie = (isEvent = false) => {
    const types = [{id:'p', d:120, t:"SPEEL 120 SECONDEN"}, {id:'u', d:5, t:"KOOP 5 UPGRADES"}, {id:'v', d:500, t:"VERDIEN $500"}];
...
window.openShop = () => {
    ...
    if (!Number.isFinite(shopUpgradePrijs) || shopUpgradePrijs < 1) shopUpgradePrijs = 1;
...
window.save = (silent = false) => { 
    ...
    const data = { 
        ...
        diamanten, shopUpgradeLevel, shopUpgradePrijs, verdienMultiplier, totaalSpeeltijdSec
    };
...
window.load = () => { 
    ...
    verdienMultiplier = d.verdienMultiplier ?? Math.pow(SHOP_MULTIPLIER_STEP, shopUpgradeLevel);
...
function animate() {
    ...
    if ((actieveOpdracht && actieveOpdracht.id === 'p') || (eventOpdracht && eventOpdracht.id === 'p')) uiDirty = true;
